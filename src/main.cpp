...
#if defined(NDEBUG)
#error "cougar cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "cougar Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 200) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[cougar] Adjusting cougar at height %d with %d masternodes (%d % locked CGR) and %d CGR supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
			LogPrintf("[cougar] Masternode: %d\n", masternodeMultiplier*100);
			LogPrintf("[cougar] Staker: %d\n", (1 - masternodeMultiplier)*100);
