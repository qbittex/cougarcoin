...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("cougarMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("cougarMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("cougarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cougar-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("cougarMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running cougarMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadcougarMiner() exception");
    } catch (...) {
        LogPrintf("ThreadcougarMiner() exception");
    }

    LogPrintf("ThreadcougarMiner exiting\n");
}
