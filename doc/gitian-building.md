...
*Setup instructions for a gitian build of cougar using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the cougar
...
- [Building cougar](#building-cougar)
...
Clone the git repositories for cougar and gitian and then checkout the cougar version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/cougarcoin/cougar-core.git
cd cougar-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cougar-core repository
...
To build cougar (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cougar-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cougar-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cougarcoin/cougar-core
...
./bin/gbuild --commit cougar=${COMMIT} --url cougar=${URL} ../cougar/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cougar=${COMMIT} --url cougar=${URL} ../cougar/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cougar=${COMMIT} --url cougar=${URL} ../cougar/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cougar-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cougar-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cougar-build.assert
```
...
[cougarcoin/gitian.sigs](https://github.com/cougarcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
